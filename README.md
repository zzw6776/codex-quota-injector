...
- 将所选账号迁移到其他设备：临时使用只交付短期 access token，完整转移交付刷新凭据并停用源设备账号；
...
面板中的“迁移账号”会在系统“下载”目录生成 JSON 文件。两种模式都会先刷新所选 OAuth 账号，并把新的 access token 和 refresh token 一起写回当前设备的加密账户库。临时使用文件只包含有时效的 access token，目标设备不能续期，源设备继续使用；完整转移文件包含完整 OAuth 凭据或 API Key，生成后源设备把账号标记为“已转出”，停止刷新、切换和唤醒。若文件尚未导入或目标设备已停止使用，可点击“已转出”验证并恢复；目标设备已轮换凭据时，源设备需要重新 OAuth。迁移文件属于明文敏感凭据，只能存放在可信设备中；目标设备仍从“Token / JSON”入口导入。
