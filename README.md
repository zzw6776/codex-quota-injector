...
在账号卡片中点击“唤醒设置”，添加一个或多个 24 小时时刻（例如 `08:00`、`13:00`、`18:00`），勾选“开启每日定时唤醒”并保存。时间按电脑本地时区每天重复，设置及最近一次结果保存在加密账号库中。API Key 账号不提供此功能。

“立即唤醒”无需开启定时，会用该账号通过独立的官方 Codex 进程发送一句简短消息。进程使用临时目录和仅存在于内存中的登录凭据，不切换客户端账号、不重启客户端，也不将唤醒对话加入日常任务。模型取该账号官方目录的默认文本模型，推理强度取其支持的最低档；调用会产生模型用量。
...
手动确认时，可以先选择一个非当前账号点击“立即唤醒”，查看请求状态、所用模型、回复及额度刷新结果，再确认该账号的重置时间是否开始新一轮计时。随后设置一个将来的时刻并保存，保持 Codex 和电脑运行，确认定时结果及重新打开程序后的设置保留情况。显示“请求完成”仅表示模型已回复，不代表已验证服务端的额度计时规则；超时或中断时结果可能未知，可手动刷新额度确认。
