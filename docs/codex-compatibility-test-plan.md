...
测试影响范围与证据复用以[测试总索引](testing-index.md#修改后测什么按实际影响复用)为准：发布版本、说明文档或无关用例变化不要求全面重测。保留原始执行版本，按组件/环境输入与实际运行时判断有效性；旧全仓摘要不再作为全部组件失效门禁。

免费回归报告前置门禁已取消。本文旧流程中的“免费报告通过后才能执行”不再适用；免费、真实模型及启停恢复测试按用户指定范围分别执行，原生环境、供应商、预算、同源码后台/桌面绑定和重启授权要求继续适用。

状态：免费契约、实际官方运行时、生产模型目录与中继、隔离浏览器、分离的真实模型测试后台/桌面执行器和独立启停恢复测试生命周期监督器已实现。免费回归已按公共、Windows 原生 Relay、WSL 原生 Relay 拆分；Windows 启停恢复测试会自动切换两套运行环境并恢复原配置。当前结果见[测试说明](testing.md)及本次运行报告；真实模型、真实桌面任务和 Windows 真机生命周期仍必须分别执行验收。本文不是整套测试通过报告。
...
| E18 | [desktop-host-evidence.test.mjs](../test/desktop-host-evidence.test.mjs) 与 [desktop-fixture.test.mjs](../runtime-tests/desktop-fixture.test.mjs) | 固定 官方模型与 DeepSeek Flash 测试后台与桌面组件不互相继承；桌面报告只从匹配随机标记的真实 rollout、模型/调用 ID、失败退出码后的续接、运行时绑定和本机 HTTP 服务记录判定，不采信模型自述。这里只验证执行器契约，不能代替真实桌面任务运行 |
...
- `free-common`：共享逻辑、状态机、Router、协议样本、故障注入和页面/浏览器契约，只执行一次。
- `free-macos-native-relay`：macOS Node.js、官方 CLI 与生产 shim/Relay 的实际组合。
- `free-windows-native-relay`：Windows Node.js、Windows 依赖与缓存、Windows 官方 CLI、实际 PE SEA Relay、Windows 路径/进程/临时目录。
- `free-wsl-native-relay`：WSL Linux Node.js、独立依赖与缓存、WSL 官方 CLI、实际 ELF SEA Relay、Linux 环境变量、路径转换与进程身份。

免费回归套件必须用临时工作区、临时配置、测试凭据和可记录请求的本地模型端点。macOS 使用 Seatbelt 将运行时限制到回环服务，临时 Chrome/Edge 额外使用模拟钥匙串；Windows 原生使用临时 HOME/APPDATA，WSL 把源码复制到 Linux 临时工作区后独立执行 `npm ci` 并使用临时 HOME、依赖与缓存。若请求未到本地服务，或者出现真实上游的网络/认证结果，测试必须失败并保留证据。联网但不调用模型的检查单独标注网络依赖。离线测试的工具结果应由真实测试文件、进程或服务产生。报告必须同时给出当前桌面运行环境状态与当前平台全部支持环境状态；公共证据可复用，原生 Relay 结果不得继承。
...
### 真实模型测试：消耗 Token 的真实验收

在免费回归通过后主动展示计划并询问。官方模型测试只运行 Codex 官方模型，DeepSeek Flash 测试只运行 DeepSeek；两批分别授权、分别生成报告，一批的同意和结果不能覆盖另一批。每次真实模型测试还必须绑定一个运行环境，默认读取当前桌面设置，也可显式指定本平台环境；脚本不修改桌面运行方式，且所选环境对应的免费回归组件必须通过。每个真实模型测试固定拆成后台和桌面两个组件：后台真实测试按工具、历史、压缩、app-server 回调四个隔离阶段运行；桌面组件让目标模型的真实 Codex 桌面任务执行 functions.exec、web.run 和 computer use，并由独立执行器读取 rollout、当前运行时及平台材料记录。Windows 使用动态生成的 WinForms 原生清单，macOS 使用本机 HTTP 材料。只有两个组件属于同一源码、平台、运行环境和供应商且都通过，整批才通过。保留独立唤醒冒烟，但不给它桌面入口验收资格。

每项真实模型测试用例必须留下输入条件、实际工具/接口、关联任务/轮次/请求 ID、终态和独立结果。比较原生与注入模式的功能结果，不要求两次模型文字逐字相同。基线复用仅限 Codex/工具/配置版本未变且有有效证据；失败或升级时重新运行对照。

### 启停恢复测试：关闭或重启 Codex 的生命周期验收

启停恢复测试在免费回归通过后列出计划并主动询问，但始终安排在 官方模型与 DeepSeek Flash 测试之后单独执行。它覆盖正式包安装和更新、进程接管、单实例、中继断线恢复、关闭重开、真实账号切换和恢复、最终状态；账号条件满足时会发送计划中披露的一次最低价官方模型冒烟。Windows 启停恢复测试要求完整 Windows 免费回归，自动保存原配置、切到 Windows 原生执行完整生命周期、切到 WSL 原生重复执行，再恢复测试前的运行方式并保留 Codex 同期写入的其他配置，最后做账号往返；用户不手动切换。调度时记录发起任务及回合；任何可能关闭 Codex、注入器或 Relay 的步骤先读取官方 rollout 事件，等待所有活动回合完成或中止并稳定落盘；等待超时就拒绝关闭。首次关闭前检查 rollout ordinal 连续性、旧回合终态和当前分页投影。重复 ordinal 或缺失中断终态只有在对话内容摘要保持一致、相关进程已停止且原 rollout 和 Windows/WSL SQLite 已备份时自动修复，并只清空目标任务的派生投影；未知结构损坏保持阻塞。每套运行环境只在桌面停止期间通过正式包对应的原生 Relay 显式恢复落后的任务，使用独立中继状态文件；重建进程退出后才启动桌面。启动后的门禁只读确认发起回合已成为持久化终态、分页投影追平 rollout，并且首条用户消息与完成回合的最终助手消息存在；不允许并发启动第二个 app-server，最终仍未追平则停止后续重启。重复启动只把桌面主进程 PID 作为 Codex 单实例身份，app-server PID 单独记录并允许独立轮换。调度前必须先打开独立实时报告页，macOS 使用 launchd、Windows 使用带登录触发和失败重启策略的任务计划程序，使 Codex 关闭后测试仍可继续并留下最终报告。所有平台不备份旧安装、不核对旧版本，失败时直接停止并保存实际失败；正常步骤仍测试运行方式恢复与账号回切。监督器到达任一终态后重试将 Codex 窗口置前，把结果与发起任务 ID 写入报告；该通知结果不参与启停恢复测试的通过判定。仍在运行的验收控制器不能重复调度。各平台的旧回滚失败报告保留原结论，但不再阻塞新验收；已移除 `--recover=<run-id>` 入口。macOS 同样移除旧应用备份、失败回滚和旧包核对。
...
```mermaid
flowchart LR
    启停恢复测试[当前 Codex 对话] -->|准备计划、读取结果| R[独立测试运行器]
...
每行是稳定场景编号。用例名称标记场景编号，免费回归与真实模型测试由所属测试批次区分；一行可以有多个有意义的测试。与共用底层路径有关的工具复用契约用例，各自独特的执行行为保留验收。表中的“现有证据/缺口”为最初 E 编号盘点，当前实施和执行状态以[场景索引](testing-scenarios.json)及生成报告为准，不能按历史缺口推断尚未补测。

### 4.0 测试运行器与隔离前置验收

这些场景先于会重启、接管、切换账号或注入故障的测试执行。它们仍按当前平台验证，不要求部署其他平台环境。

| 编号 | 场景与触点 | 现有证据/缺口 | 免费回归：免费验证 | 真实模型测试：真实判据 |
| --- | --- | --- | --- | --- |
| HAR-01 | 目标隔离与所有权；运行器、platform、单实例、凭据 | 目前无整体验证 | 确认全部目标、配置、端口与宿主；在受控环境验证生产停止/接管范围；不能指向日常实例 | 前置条件不成立时禁止启动会影响日常实例的真实模型测试场景 |
...
| TOOL-04 | 动态工具、桌面工具、exec 编排与宿主回调；RPC、辅助进程 | E10 仅账本 | item/tool/call、currentTime/read 等宿主请求、命名空间、嵌套调用、结果类型与错误；记录实际注册源；解析桌面证据时验证 `codex_app` 四个常用只读入口的调用结果、会话读取顺序、当前任务 ID 和真实消息内容；每个完成回合必须同时有用户和助手消息，随机标记由 rollout 独立绑定 | 主入口先用 `list_threads` 找到当前任务，再以返回 ID 调用 `read_thread` 并读回真实消息；任一完成回合的 `items` 为空都不能通过，当前活动回合可以为空；`list_projects` 与 `get_usage_limits` 正常返回；本机其他宿主工具完成真实操作；调用未发生时不能算通过 |
...
| TOOL-06 | 桌面 computer use；辅助 CLI、工具宿主、原生应用或页面 | 没有完整执行验收 | 公共页面契约及 Windows 原生 WinForms 材料的随机标记、输入、单次提交、独立证据和清理 | Windows 主入口操作每轮动态生成的非浏览器原生应用；macOS 操作仅绑定本机回环地址的 HTTP 材料。截图单列；Relay 阻断必须与无 Relay 官方基线对照后归因 |
| TOOL-07 | 并行工具、工具失败与恢复；RPC、Router、历史 | E05/E07/E10 局部 | 同名不同 ID、结果乱序、部分失败、工具取消、较大返回；每条结果保持关联 | 并行任务得到各自结果；一次工具失败后其他结果仍可使用 |
| INT-02 | 用户补充输入与 MCP elicitation；双向 RPC、任务状态 | 尚无完整流程 | Plan 用户输入完成请求→回答→serverRequest/resolved；等待中中断/关闭/追加轮次不遗留等待计数；MCP elicitation 在 `never` 下直接终结 | 不纳入固定真实模型测试桌面验收；需要时单独执行 |
...
| IO-04 | 实时语音、文本/音频追加和结束；realtime 协议、桌面音频宿主 | 协议有分支，现无验收 | 输入样本、会话生命周期、转录/音频事件、打断、设备不可用与清理 | 当前配置支持时实际往返并结束会话，验证音频/转录结果；单向事件回放只算免费回归 |
...
| UI-02 | 面板全部动作；widget→动作队列→manager | E08 drain 表达式；各 manager 局部 | 实际点击/输入/保存/取消，覆盖账号迁移与恢复、刷新、上下文、模型管理中的 DeepSeek 预设和其他平台、唤醒动作；所有面板在关闭按钮左侧固定显示 `codex_app` 彩色状态点（绿/黄/红/蓝对应正常/启动中/异常/直连），不显示状态文字且使用普通指针；正常 Tooltip 只显示易读功能名，异常时显示缺失项、建议和技术诊断；轮询时间变化不得触发面板重绘或打断悬浮；降级时验证常驻诊断及重新检查/重启/打开日志动作；断言事件恰好执行一次和持久化结果 | 需要模型的动作按对应真实模型测试场景验证；真实工具缺失必须在 UI 可见，不能仅验证按钮存在 |
...
免费回归通过测试配置覆盖当前平台所有可达生产分支。真实模型测试验证本次声明适用的真实 profile；缺少真实供应商凭据记“配置未验证”，报告不得把它写成已验证。每个支持的能力域都要有执行证据；通用契约可复用，不复制每个工具名字的相同字段断言。组合膨胀时按共同路径归组并说明等价依据；启动分支、协议转换分支、工具宿主差异不能合并掉。

其他 OS/架构为范围外，不阻塞当前平台结果。macOS 完整免费回归为 `free-common + free-macos-native-relay`；Windows 完整免费回归为 `free-common + free-windows-native-relay + free-wsl-native-relay`。Windows 的定向开发运行可以只选当前模式，但只能形成 `currentRuntimeStatus`，不能把 `allSupportedStatus` 标为通过。官方模型与 DeepSeek Flash 测试各自只选一个运行环境；Windows 启停恢复测试才自动切换并完整验证两套正式入口。正式产物验收只要求当前平台/架构对应的产物。

## 6. 实施结构与每次执行流程

当前入口为免费回归的 `npm run test:offline`、官方模型与 DeepSeek Flash 测试后台的 `npm run test:live:official -- --plan` / `npm run test:live:deepseek -- --plan`、官方模型与 DeepSeek Flash 测试桌面的 `npm run test:desktop -- --profile=<official|deepseek> --plan` 和启停恢复测试的 `npm run test:lifecycle -- --plan`。官方模型与 DeepSeek Flash 测试分别取得当次授权后追加 `--confirm-token-use`，启停恢复测试单独取得当次授权后使用 `--confirm-restart`；原有 `npm test` 保留为免费基础回归。
...
| 浏览器与桌面宿主 | 免费回归使用真实 Chrome/CDP 和生产 Widget；真实模型测试按 `docs/testing-desktop-host.md` 运行 `npm run test:desktop`，在操作前打开实时报告，并只在验收期间创建平台材料；Windows 动态生成原生 WinForms 应用，macOS 提供本机 HTTP 页面。材料不驱动测试，判定来自 rollout、当前运行时和独立证据 |
...
2. 接入不影响日常实例的官方运行时组合、工具执行和当前平台页面验证，逐域补齐免费回归；Windows 原生与 WSL 原生分别使用实际运行时、依赖、CLI 和 Relay，没有本环境执行证据时不标记该组件通过。
3. 完整免费回归通过后，主动展示官方模型测试、DeepSeek Flash 测试、启停恢复测试的计划并询问用户分别执行哪些批次；不等待用户再次提出测试要求。
4. 经各自当次授权，先在当前或明确指定的单个运行环境执行 官方模型与 DeepSeek Flash 测试后台组件，再运行同一供应商的桌面组件；两份报告绑定一致且都通过后才标记该真实模型测试批通过。此阶段不关闭客户端、不重启、不切换账号。
5. 最后执行启停恢复测试。入口先在浏览器打开由脱敏报告生成的只读进度页，打开失败就不调度重启；macOS 的 launchd 或 Windows 的任务计划程序让监督器独立落盘和刷新页面。Windows 自动切换两套原生 Relay 并恢复原配置，未知的模型请求结果不重放，失败时回切原账号并恢复安装前状态。
6. 代码、配置、测试或规则改动后不自动运行完整 `npm run test:offline`；只有用户明确要求免费回归时才运行指定范围。官方模型测试、DeepSeek Flash 测试、启停恢复测试始终与免费回归分开。

首次建立真实模型测试时实际测量各场景输入/输出、工具费用与耗时，形成每次运行可审查的预算。不要承诺未经测量的固定 Token 成本。设定调用次数、时间及用量停止条件；不通过重复重试消耗预算来掩盖失败。超出预算的剩余场景记未执行，不能输出全通过。
...
官方模型与 DeepSeek Flash 测试报告还必须给出 `backendStatus`、`desktopHostStatus` 和 `overallStatus`。后台通过但桌面未运行时为 `incomplete`；不同平台、运行环境、供应商、源码摘要或历史版本的桌面证据不能合并。桌面执行器固定核对的核心链通过，也不会把未配置或未执行的 Apps、媒体、自动化、远程和语音场景自动改为通过。

通过结论示例：`macOS arm64 / Codex 26.903.71938 / CLI 0.153.4 / C1+C2+C3 / 正式入口：适用免费回归、真实模型测试场景通过`。如果只完成免费回归，结论必须注明“免费回归通过，真实验收未完成”。范围内的 FAIL、BLOCKED、NOT_RUN、INTERRUPTED 不能被总成功数遮住。基线也失败时保留“上游/环境待定位”，不自动认定注入器无问题。
...
已实现原有基础回归、E13–E17 公共链路回归、实际官方运行时/生产目录/shim/relay/Router/Chat 组合、任务状态、交互、插件/Hooks、实际浏览器、账号异常回归及 macOS/Windows 生命周期监督器，并提供免费与真实测试入口、代码/运行时摘要门禁、独立可视进度页和逐场景证据报告。Windows 免费回归已拆成公共、Windows 原生和 WSL 原生组件；真实模型测试绑定单个运行环境；Windows 启停恢复测试自动切换、验证并恢复两套正式入口。macOS 已有实际执行报告；Windows 适配器和安装包检查仍须在 Windows 真机运行，不能由 macOS 结果代替。详见[测试说明](testing.md)。本轮源码版本、中继协议及其实际加载状态以当前包和 lifecycle 报告为准。
