...
C 在 A 通过后列出计划并主动询问，但始终安排在 B1/B2 之后单独执行。它覆盖正式包安装和更新、进程接管、单实例、中继断线恢复、关闭重开、真实账号切换和恢复、最终状态；账号条件满足时会发送计划中披露的一次最低价官方模型冒烟。Windows C 要求完整 Windows A，自动保存原配置、切到 Windows 原生执行完整生命周期、切到 WSL 原生重复执行，再恢复测试前的运行方式并保留 Codex 同期写入的其他配置，最后做账号往返；用户不手动切换。调度时记录发起任务及回合；任何可能关闭 Codex、注入器或 Relay 的步骤先读取官方 rollout 事件，等待所有活动回合完成或中止并稳定落盘；等待超时就拒绝关闭。首次关闭前检查 rollout ordinal 连续性、旧回合终态和当前分页投影。重复 ordinal 或缺失中断终态只有在对话内容摘要保持一致、相关进程已停止且原 rollout 和 Windows/WSL SQLite 已备份时自动修复，并只清空目标任务的派生投影；未知结构损坏保持阻塞。每套运行环境只在桌面停止期间通过正式包对应的原生 Relay 显式恢复落后的任务，使用独立中继状态文件；重建进程退出后才启动桌面。启动后的门禁只读确认发起回合已成为持久化终态、分页投影追平 rollout，并且首条用户消息与完成回合的最终助手消息存在；不允许并发启动第二个 app-server，最终仍未追平则停止后续重启。重复启动只把桌面主进程 PID 作为 Codex 单实例身份，app-server PID 单独记录并允许独立轮换。调度前必须先打开独立实时报告页，macOS 使用 launchd、Windows 使用带登录触发和失败重启策略的任务计划程序，使 Codex 关闭后测试仍可继续、失败时可回滚并留下最终报告。监督器到达任一终态后重试将 Codex 窗口置前，把结果与发起任务 ID 写入报告；该通知结果不参与 C 的通过判定。若存在未完成或回滚失败的旧 C，新任务必须停止并给出恢复或检查命令；`--recover=<run-id>` 只重试失败的回滚，恢复成功后仍保留原测试失败结论。
...
| INT-02 | 用户补充输入与 MCP elicitation；双向 RPC、任务状态 | 尚无完整流程 | Plan 用户输入完成请求→回答→serverRequest/resolved；等待中中断/关闭/追加轮次不遗留等待计数；MCP elicitation 在 `never` 下直接终结 | 不纳入固定 B 桌面验收；需要时单独执行 |
...
| 浏览器与桌面宿主 | A 使用真实 Chrome/CDP 和生产 Widget；B 按 `docs/testing-desktop-host.md` 运行 `npm run test:desktop`，在操作前打开实时报告，并只在验收期间创建平台材料；Windows 动态生成原生 WinForms 应用，macOS 提供本机 HTTP 页面。材料不驱动测试，判定来自 rollout、当前运行时和独立证据 |
