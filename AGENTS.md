...
- 真实桌面宿主的 functions.exec、web.run、computer use 和用户补充输入由 `test:desktop` 固定留证；执行器还必须从模型请求层记录只含类型、名称和命名空间的脱敏工具清单，逐项区分 `passed`、`unsupported`、`not-executed` 与 `failed`。B1 的 web.run 是必需能力；B2 只有独立 `web.run` 工具时才执行该项，DeepSeek Responses 请求中存在但供应商会忽略的 Hosted `web_search` 描述不能算可调用能力，应按 `unsupported` 留证并阻断 B2 桌面组件，不反复诱导模型重试。通过跨任务委托驱动验收时可用 `--trigger-mode=delegated`，此时 read_thread 只要求成功读取正确任务，随机标记仍由 rollout 独立绑定；直接模式继续要求 read_thread 输出标记。Apps、语音、媒体、自动化等条件能力按 `docs/testing-desktop-host.md` 归入其所使用模型对应的 B1 或 B2 并单独保留状态。自有动态工具、后台 app-server 或固定桌面链通过都不能把未执行的条件能力标绿。审批允许/拒绝弹窗不属于测试项，项目固定使用 `never` 策略。
