...
for (const profile of profiles) test(`[B MOD-03 TOOL-01 TOOL-03 TOOL-07] ${profile.id} 真实工作区与 MCP 工具`, { timeout: 180_000 }, async t => {
  ...
  try {
    ...
    stage = "MCP 就绪与直调预检";
    await writeFile(join(r.cwd, "mcp-marker.txt"), marker);
    const mcpThread = (await r.thread({ approvalPolicy: "never" })).thread;
    const status = await r.rpc.request("mcpServerStatus/list", {
      threadId: mcpThread.id,
      detail: "toolsAndAuthOnly",
    });
    assert.match(JSON.stringify(status), /read/, "fixture MCP 未发现 read 工具");
    ...
    assert.match(JSON.stringify(direct), new RegExp(marker));

    stage = "模型驱动 MCP 调用";
    const beforeMcp = r.rpc.events.length;
    const mcpReply = await r.turn(mcpThread.id,
      `必须实际调用名为 mcp__fixture__read 的只读工具一次。工具返回口令后只回复该口令，不要调用其他工具。`);
    const mcpItem = r.rpc.events.slice(beforeMcp).find(event =>
      event.method === "item/completed" && event.params?.item?.type === "mcpToolCall" &&
      event.params.item.server === "fixture" && event.params.item.tool === "read")?.params.item;
    assert.ok(mcpItem, "没有模型实际调用 MCP read 的事件");
    assert.equal(mcpItem.status, "completed",
      `模型 MCP read 调用失败：${r.sanitize(mcpItem.error?.message ?? "无错误详情")}`);
    ...
    assert.equal(mcp.filter(event =>
      event.method === "tools/call" && event.params.name === "read").length, 2,
